...
/--
  ...
  This is an interpretation of theorem 2.1 from Chapter 1. Another interpretation
  could be `DNF.𝓢_functional_complete`.

  The top symbol (`T`) is needed since the boolean function with no arguments
  that evaluates to true cannot be represented by a formula with no variables.
  The reason `T` is not needed for `DNF.𝓢_functional_complete` is because the
  `⋀` (big and) symbol is of variable arity, and for arity zero is defined as
